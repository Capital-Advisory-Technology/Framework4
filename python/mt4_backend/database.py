...
class DatabaseManager:
    ...
    async def get_unprocessed(self) -> list[dict]:
        query = "SELECT * FROM backtests where is_processed = 0"
    ...
    async def get_or_create_backtest(self, strategy_id, backtest: dict) -> int:
        ...
        insert_query = (f"INSERT INTO models "
                        f"(strategy_id, bt_raw_id, symbol_id, period_id, account_currency, inputs) "
                        f"VALUES "
                        f"({strategy_id}, {bt_id}, {symbol_id}, {period_id}, '{account_currency}', '{inputs}')")
    ...
    async def save_stats(self, bt_id: int, backtest: dict, stats: dict):
        ...
        get_query = (f"SELECT * FROM model_stats "
        ...
        stats_query = (f"INSERT INTO model_stats "
        ...
        update_query = f"UPDATE backtests SET is_processed = 1 WHERE backtest_id = {backtest['bt_raw_id']}"
        await self.db.execute(update_query)

        await self.db.commit()
        logger.info(f"Stats for backtest {bt_id} saved.")

    async def get_or_create_model(self, strategy_id, backtest):
        bt_id = backtest["backtest_id"]
        symbol_id = await self.get_symbol_id(backtest["symbol"])
        period_id = await self.get_period_id(backtest["period"])
        account_currency = backtest["account_currency"]
        ...
        get_query = f"SELECT * FROM models WHERE strategy_id = {strategy_id}"
        cursor = await self.db.execute(get_query)
        row = await cursor.fetchone()
        if row:
            return row[0]
