...
extern color           ColorUp         = clrGreen;    // Color for up
extern color           ColorDn         = clrWhite;      // Color for down
...
#define _mtfCall(_buff,_y) iCustom(NULL,TimeFrame,indicatorFileName,PERIOD_CURRENT,MaPeriod,MaFilterPass,0,Deviation,Price,_buff,_y)
...
int init()
  {
   ...
   for(int i=0; i<4; i++)
      SetIndexShift(i,MaShift*TimeFrame/_Period);
   ...
   for(i=limit; i>=0; i--)
     {
      ...
      if(i<Bars-1)
        {
         if(trend[i]==-1 && smax[i]>smax[i+1])
            smax[i] = smax[i+1];
         if(trend[i]== 1 && smin[i]<smin[i+1])
            smin[i] = smin[i+1];
        }
      upb[i] = EMPTY_VALUE;
      dnb[i] = EMPTY_VALUE;
...
double iMultiPassMa(double price, int period, int filterPass, int i, int instanceNo=0)
  {
   if(ArrayRange(workMpFilter,0) != Bars)
      ArrayResize(workMpFilter,Bars);
   i = Bars-i-1;
