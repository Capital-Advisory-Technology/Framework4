...
//|                                              http://www.mql4.com |
//+------------------------------------------------------------------+
#property description "Average True Range Percentage"
#property strict
...
//--- name for DataWindow and indicator subwindow label
   short_name="ATRP("+IntegerToString(InpAtrPeriod)+")";
...
//--- the main loop of calculations
   for(i=limit; i<rates_total; i++)
     {
      ExtTRBuffer[i]=(MathMax(high[i],close[i-1])-MathMin(low[i],close[i-1])) / close[i-1] * 100;
      ExtATRBuffer[i]=(ExtATRBuffer[i-1]+(ExtTRBuffer[i]-ExtTRBuffer[i-InpAtrPeriod])/InpAtrPeriod);
      // ExtATRBuffer[i] = (ExtATRBuffer[i] / close[i]) * 100;
     }
